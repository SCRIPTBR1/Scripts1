...
-- Clean old GUIs
for _, name in pairs({"BRHubDuelsGUI", "PulseHubDuelsGUI"}) do
...
-- 🔴⚫⚪ CORES EXCLUSIVAS: Vermelho, Preto e Branco
local _C = {
	[1] = Color3.fromRGB(10, 0, 0),     -- BG escuro
	[2] = Color3.fromRGB(18, 0, 0),     -- Sidebar
	[3] = Color3.fromRGB(28, 2, 2),     -- Card BG
	[4] = Color3.fromRGB(45, 5, 5),     -- Card Hover
	[5] = Color3.fromRGB(160, 10, 10),  -- Borda vermelha
	[6] = Color3.fromRGB(220, 20, 20),  -- Borda vermelha brilhante
	[7] = Color3.fromRGB(255, 255, 255),-- Branco
	[8] = Color3.fromRGB(200, 50, 50),  -- Vermelho claro
	[9] = Color3.fromRGB(60, 8, 8),     -- Vermelho escuro
	[10] = Color3.fromRGB(12, 0, 0),    -- Input BG
}
local BG = _C[1]
...
mainStroke.Color = Color3.fromRGB(180, 10, 10)
...
topbar.BackgroundColor3 = Color3.fromRGB(8, 0, 0)
...
verLbl.Position = UDim2.new(0, 120, 0, 0)
verLbl.BackgroundTransparency = 1
verLbl.Text = "💎 VIP"
verLbl.TextColor3 = BORDER2
...
minBtn.BackgroundColor3 = Color3.fromRGB(18, 0, 0)
...
minBtn.MouseEnter:Connect(function() TweenService:Create(minBtn, TweenInfo.new(0.1), {BackgroundColor3=Color3.fromRGB(40,0,0)}):Play() end)
minBtn.MouseLeave:Connect(function() TweenService:Create(minBtn, TweenInfo.new(0.1), {BackgroundColor3=Color3.fromRGB(18,0,0)}):Play() end)

local portrait = Instance.new("Frame", main)
portrait.Size = UDim2.new(0, PW, 1, -44)
portrait.Position = UDim2.new(0, 0, 0, 44)
portrait.BackgroundColor3 = Color3.fromRGB(6, 0, 0)
...
portImg.Image = "rbxassetid://137966160902234"
...
nameLine.Size = UDim2.new(0.7, 0, 0, 2)
nameLine.Position = UDim2.new(0, 8, 1, -30)
nameLine.BackgroundColor3 = BORDER2
...
byTag.Text = "💎 VIP | discord.gg/envyhub"
byTag.TextColor3 = BORDER2
...
mini.BackgroundColor3 = Color3.fromRGB(10, 0, 0)
mini.BorderSizePixel = 0
mini.Text = "🌹 BR HUB DUELS"
...
miniStroke.Color = Color3.fromRGB(180, 10, 10)
...
mini.MouseEnter:Connect(function() TweenService:Create(mini,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(24,0,0)}):Play() end)
mini.MouseLeave:Connect(function() TweenService:Create(mini,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(10,0,0)}):Play() end)
...
local ACTIVE_TAB_BG  = Color3.fromRGB(60, 5, 5)
local ACTIVE_TAB_TXT = WHITE
local IDLE_TAB_BG    = Color3.fromRGB(16, 0, 0)
...
for i, td in ipairs(tabDefs) do
	...
	btn.MouseEnter:Connect(function()
		if activeTabName ~= td.name then TweenService:Create(btn, TweenInfo.new(0.1), {BackgroundColor3=Color3.fromRGB(35, 2, 2)}):Play() end
	end)
...
local function makePillToggle(parent, defOn, onToggle)
	...
	pbg.BackgroundColor3 = defOn and WHITE or Color3.fromRGB(16, 0, 0)
	...
	local function setV(on)
		isOn = on
		TweenService:Create(pbg, TweenInfo.new(0.18), {BackgroundColor3=on and WHITE or Color3.fromRGB(16, 0, 0)}):Play()
...
pbFrame.BackgroundColor3 = Color3.fromRGB(9, 0, 4)
pbFrame.BorderSizePixel = 0
pbFrame.Active = true
Instance.new("UICorner", pbFrame).CornerRadius = UDim.new(0, 10)
Instance.new("UIStroke", pbFrame).Color = Color3.fromRGB(180, 10, 10)
...
saveConfig = function(btn)
	...
	local ok = pcall(function()
		...
		if writefile then writefile("BRHubConfig.json", encoded) end
	end)
	if not ok then
		pcall(function()
			local encoded = HttpService:JSONEncode(cfg)
			if _writefile then _writefile("BRHubConfig.json", encoded) end
		end)
	end
	if btn then
		local prev = btn.Text
...
local function teardownDesyncPhys()
	...
	for _, inst in ipairs(workspace:GetChildren()) do
		if inst.Name == "BRHub_DesyncPos" then pcall(function() inst:Destroy() end) end
	end
...
local function setupDesyncCharacter(char)
	...
	DS.invisiblePart.Name = "BRHub_DesyncFollower"
	...
	local dhAtt = Instance.new("Attachment"); dhAtt.Name = "BRHub_DesyncDrive"; dhAtt.Parent = DS.invisiblePart
	local lv = Instance.new("LinearVelocity"); lv.Name = "BRHub_DesyncLinearVelocity"
	...
	local welded = false
	pcall(function()
		local w = Instance.new("Weld"); w.Name = "BRHub_DesyncWeld"
	...
	if not welded then
		...
		local wc = Instance.new("WeldConstraint"); wc.Name = "BRHub_DesyncWeldConstraint"
...
local CONFIG_FILE="BRHubConfig.json"
...
local Presets={}
local PRESET_FILE="BRHubPresets.json"; local LAST_PRESET_FILE="BRHubLastPreset.json"
                ...
                for _, child in ipairs(page:GetChildren()) do
                    if child:IsA("Frame") then
                        for _, subchild in ipairs(child:GetChildren()) do
                            if subchild:IsA("TextBox") and subchild.Parent.Name ~= "BRHubDuelsGUI" then
...
local function setupOtherPlayerBillboard(player)
    ...
    local function addBillboard(char)
        ...
        local oldBB = head:FindFirstChild("BRHubOtherBB")
        if oldBB then oldBB:Destroy() end
        
        local bb = Instance.new("BillboardGui", head)
        bb.Name = "BRHubOtherBB"
...
local function setupChar(char)
    ...
    local head=char:FindFirstChild("Head")
    if head then
        local oldBB=head:FindFirstChild("BRHubBB"); if oldBB then oldBB:Destroy() end
        local bb=Instance.new("BillboardGui",head); bb.Name="BRHubBB"
        ...
        discordLbl.BackgroundTransparency=1; discordLbl.Text="💎 BR HUB VIP"
...
print("[🌹 BR HUB Duels] Loaded successfully! | 💎 VIP Edition")
...
task.spawn(function()
	...
	LP.CharacterAdded:Connect(function(newChar)
		...
		if State.laggerToggled then
			...
			laggerPhase = 0
			if mobileLaggerSetActive then mobileLaggerSetActive(false) end
			if modeValLbl then modeValLbl.Text = "None" end
		end
		...
		print("[🌹 BR HUB] 🛡️ SAFE MODE: All features disabled on respawn!")
	end)
end)

task.spawn(function()
	local function applyNeonOutline(obj)
		if obj:FindFirstChild("__BRHubNeon") then return end
		local stroke = Instance.new("UIStroke", obj)
		stroke.Name = "__BRHubNeon"
		...
		local function scan(root)
			for _, d in ipairs(root:GetDescendants()) do
				if d:IsA("TextButton") or d:IsA("ImageButton") then
					pcall(applyNeonOutline, d)
				end
			end
		end
		pcall(function() if main then scan(main) end end)
		pcall(function() if mini then applyNeonOutline(mini) end end)
		pcall(function() if MobilePanel then scan(MobilePanel) end end)
		if gui then
			gui.DescendantAdded:Connect(function(d)
				if d:IsA("TextButton") or d:IsA("ImageButton") then
					task.wait(0.05); pcall(applyNeonOutline, d)
				end
			end)
		end
	end)
...
task.spawn(function()
    ...
    local ok, err = pcall(function()
        ...
        local playlist = {
            { name = "King Nasir", id = "rbxassetid://76650356472656" },
        }
        local currentIdx = 1
        local nasirSound = nil
        local playerGui = nil
        local panel, title, pauseBtn, nextBtn

        local function ensureUI()
            if playerGui and playerGui.Parent then return end
            playerGui = Instance.new("ScreenGui")
            playerGui.Name = "BRHubMusicPlayer"
            playerGui.ResetOnSpawn = false
            playerGui.IgnoreGuiInset = true
            playerGui.DisplayOrder = 50
            pcall(function() playerGui.Parent = (gethui and gethui()) or CoreGui end)

            panel = Instance.new("Frame", playerGui)
            panel.Size = UDim2.new(0, 230, 0, 56)
            panel.Position = UDim2.new(0, 18, 1, -82)
            panel.BackgroundColor3 = Color3.fromRGB(18, 0, 4)
            panel.BackgroundTransparency = 0.05
            panel.BorderSizePixel = 0
            Instance.new("UICorner", panel).CornerRadius = UDim.new(0, 10)
            local st = Instance.new("UIStroke", panel)
            st.Color = Color3.fromRGB(220, 0, 0); st.Thickness = 1.5; st.Transparency = 0.3
            ...
            pauseBtn = makeBtn("Pause", 12)
            nextBtn = makeBtn("Next", 84)

            pauseBtn.MouseButton1Click:Connect(function()
                if not nasirSound then return end
                pcall(function()
                    if nasirSound.IsPlaying then nasirSound:Pause(); pauseBtn.Text = "Play"
                    else nasirSound:Resume(); pauseBtn.Text = "Pause" end
                end)
            end)
        end

        if State and State.kingNasirEnabled then
            task.wait(1)
            pcall(ensureUI)
        end
    end)
    if not ok then warn("[BR HUB Music] init failed:", err) end
end)
...
local function playIntroAnimation()
	...
	local pg2 = LP2:WaitForChild("PlayerGui")
	local CoreGui = game:GetService("CoreGui")

	local introGui = Instance.new("ScreenGui")
	introGui.Name = "BRHubIntroTyping"
	introGui.ResetOnSpawn = false
	introGui.IgnoreGuiInset = true
	introGui.DisplayOrder = 999
	pcall(function() introGui.Parent = (gethui and gethui()) or CoreGui end)
	if not introGui.Parent then introGui.Parent = pg2 end

	local cam = workspace.CurrentCamera
	local blur = Instance.new("BlurEffect")
	blur.Size = 0
	blur.Parent = cam
	TweenService:Create(blur, TweenInfo.new(0.6), {Size = 48}):Play()
	...
	vign.Image = "rbxassetid://6996636246"
	vign.ImageColor3 = Color3.fromRGB(180, 0, 0)
	vign.ImageTransparency = 0.4
	vign.ZIndex = 2

	local box = Instance.new("Frame", introGui)
	box.Size = UDim2.new(0, 720, 0, 140)
	box.Position = UDim2.new(0.5, -360, 0.5, -70)
	...
	label.TextStrokeColor3 = Color3.fromRGB(180, 0, 0)
	...
	sub.Position = UDim2.new(0, 0, 1, 6)
	sub.BackgroundTransparency = 1
	sub.Font = Enum.Font.Gotham
	sub.TextSize = 14
	sub.TextColor3 = Color3.fromRGB(255, 80, 80)
	sub.TextTransparency = 1
	sub.Text = "🌹 BR HUB DUELS  |  💎 VIP EDITION"
	sub.ZIndex = 6

	pcall(function()
		local snd = Instance.new("Sound", introGui)
		snd.SoundId = "rbxassetid://126107591945718"
		snd.Volume = 0.6
		snd.TimePosition = 34
		snd:Play()
		task.delay(8, function() pcall(function()
			TweenService:Create(snd, TweenInfo.new(0.5), {Volume = 0}):Play()
			task.wait(0.6); snd:Stop(); snd:Destroy()
		end) end)
	end)

	local letters = {"🌹","B","R"," ","H","U","B"," ","D","U","E","L","S"}
	local shown = ""
	for _, ch in ipairs(letters) do
		shown = shown .. ch
		label.Text = shown .. "|"
		pcall(function()
			local k = Instance.new("Sound", introGui)
			k.SoundId = "rbxassetid://6895079853"
			k.Volume = 0.4
			k:Play()
	...
	TweenService:Create(vign, fade, {ImageTransparency = 1}):Play()
